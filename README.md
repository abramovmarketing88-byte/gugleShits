...
**Подробная пошаговая инструкция:** см. **[DEPLOY.md](DEPLOY.md)** — что куда залить в Apps Script, какие листы создать, какие заголовки и SETTINGS нужны.

Кратко:
1. Откройте Google Таблицу.
2. Создайте листы **INPUT**, **OUT**, **SETTINGS**, **LOG** (заголовки — см. DEPLOY.md).
3. Перейдите в **Расширения → Apps Script**, добавьте файлы **main.gs**, **utils.gs**, **prompts.gs**, **openrouter.gs** и **CopyHtmlDialog.html** (код из репозитория).
4. Заполните SETTINGS (обязательно **OPENROUTER_API_KEY**).
5. Сохраните, обновите таблицу — в меню появится **Avito AI** (лист CONTROL создаётся при первом открытии).
